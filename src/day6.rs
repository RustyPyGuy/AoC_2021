...
pub fn day6_challenge2(config: &Config) -> Result<usize, Error> {
    ...
    for fish in lanternfish.iter() {
        ...
        match fish {
            0 => {
                lanternfish_population[0] += 1;
            }
            1 => {
                lanternfish_population[1] += 1;
            }
            2 => {
                lanternfish_population[2] += 1;
            }
            3 => {
                lanternfish_population[3] += 1;
            }
            4 => {
                lanternfish_population[4] += 1;
            }
            5 => {
                lanternfish_population[5] += 1;
            }
            6 => {
                lanternfish_population[6] += 1;
            }
            7 => {
                lanternfish_population[7] += 1;
            }
            8 => {
                lanternfish_population[8] += 1;
            }
            _ => {
                panic!("invalid fish")
            }
    ...
    println!(
        "lanternfish population post-growth, {:?}",
        lanternfish_population_to_6
    );
    println!(
        "total lanternfish population post-growth, {}",
        total_lanternfish
    );
