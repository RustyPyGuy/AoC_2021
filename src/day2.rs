...
fn read_by_lines_double</*'a, */R: Read>(io: R, coord: &mut /*'a */Coordinates) -> Result<&Coordinates, std::io::Error> {
...
fn read_by_lines_challenge2</*'a, */R: Read>(io: R, coord: &mut /*'a */Coordinates) -> Result<&Coordinates, std::io::Error> {
    let br = BufReader::new(io);
    for line in br.lines() {
            if let Ok(field) = line {
            let mut fields = field.split_whitespace();
           ...
           let field2: i32 = fields.next().unwrap().parse::<i32>().unwrap(); 
            match field1 {
                Some("forward") => {coord.move_x_with_aim(field2);},
                Some("up") => {coord.set_aim_up(field2);},
                Some("down") => {coord.set_aim_down(field2);},
                Some(_) => {println!("Failure with word pattern match.");},
                None => {println!("General failure with word parsing");},
            }
        }
...
pub fn day2_challenge1(config: &Config) -> Result<i32, Error> {
    ...
    read_by_lines_double(f,&mut submarine_coordinates);
