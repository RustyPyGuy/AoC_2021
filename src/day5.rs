...
impl Mapper {
    pub fn new() -> Result<Mapper, &'static str> {
        ...
        Ok(Mapper{mapped_points: mapped_area})
    }
    pub fn draw_on_map(&mut self, input: &Coord) /*-> Result<DangerLines, &'static str>*/ {
        if input.c1.0 == input.c2.0 { //Vertical line. Same X value
            let x = input.c1.0; 
        ...
        if input.c1.1 == input.c2.1 { //Horizontal line. Same Y value
            let y = input.c1.1; 
    ...
    pub fn draw_on_map_diagonal(&mut self, input: &Coord) /*-> Result<DangerLines, &'static str>*/ {
        if input.c1.0 != input.c2.0 && input.c1.1 == input.c2.1 { // Basic Diagonal test
            // let x = input.c1.0; 
            // if input.c1.1 > input.c2.1 {
            //     for y in input.c2.1..=input.c1.1{ 
            //     let mut temp_val:   usize =  self.mapped_points[x][y];
            ...
            //     self.mapped_points[x][y] = temp_val;
            // println!("mapped value vertical {}",self.mapped_points[x][y]);
            // // std::mem::replace(&mut vec[x][y], SomeType { some_value: 111 });
            //     }
            // }
            // else {
            //     for y in input.c1.1..=input.c2.1 { 
...
pub fn day5_challenge1(config: &Config) -> Result<i32, Error> {
    ...
    for line in lines.iter() {
        ...
        input_integers = line
            ...
            .collect::<Vec<usize>>();
        let one_coord = Coord::add(input_integers[0],input_integers[1],
            input_integers[2],input_integers[3]);
...
pub fn day0_challenge2(config: &Config) -> Result<i32, Error> {
    ...
    // println!("2 overlapped");
    // for row in sea_map.mapped_points.pop_row().unwrap() {
    //     println!("{:?}", row);
    // }
